<template>
  <div class="p-[20px] flex flex-col gap-[20px] relative">
    <div class="flex flex-col gap-[10px]">
      <div
        class="text-[32px] font-bold leading-[35px] text-[#050400] font-[Sans-serif]"
      >
        Review our Portfolio
      </div>
      <div class="text-[16px] font-semibold leading-[24px] text-[#3c3b32]">
        Bigo Real Estate offers a portfolio as dynamic as your aspirations.
        Browse through our curated categories to discover the ideal property
        that aligns with your vision for a dream home or rewarding investment.
      </div>
      ...
      <div v-else class="relative">
        <div class="overflow-hidden">
          <div
            ...
            :style="{
              gridTemplateColumns: `repeat(${paddedProperties.length}, ${
                100 / propertiesPerPage
              }% )`,
              transform: `translateX(-${
                (currentIndex / propertiesPerPage) * 100
              }%)`,
            }"
          >
            <div
            ...
            >
              ...
              >
                <img
                  :src="
                    property.images.find((img:Image) => img.is_primary)?.image_url ||
              ...
              <div
                class="flex bg-black text-orange-500 w-[40%] h-[35px] font-[Sans-serif] items-center justify-center"
              >
                Available
              </div>
              ...
              <div class="flex flex-row justify-between font-[Sans-serif]">
                ...
                to = "/contact"
                  ...
                  class="flex bg-orange-500 text-black w-[40%] h-[35px] items-center justify-center cursor-pointer hover:bg-orange-600 transition-colors"
                >
                  {{
    ...
    >
      <button
        ...
        :class="{
          'bg-orange-500':
            Math.floor(currentIndex / propertiesPerPage) === index,
...
const paddedProperties = computed(() => {
  ...
  const totalSlides = Math.ceil(
    properties.value.length / propertiesPerPage.value
  );
  const repeatCount = Math.max(2, totalSlides);
  return Array(repeatCount)
    .fill(properties.value)
    .flat()
    .slice(0, repeatCount * properties.value.length);
});
...
const prevSlide = () => {
  resetCarouselTimer();
  currentIndex.value =
    (currentIndex.value - propertiesPerPage.value + properties.value.length) %
...
const goToSlide = (slideIndex: number) => {
  resetCarouselTimer();
  currentIndex.value =
    (slideIndex * propertiesPerPage.value) % properties.value.length;
};
